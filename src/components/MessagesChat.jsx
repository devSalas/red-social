...
const MessagesChat = ({ socket }) => {
  ...
  return (
    <div className={`${(showChatMobile) ? 'flex' : 'hidden'} w-full h-full sm:flex flex-col bg-slate-200  sm:bg-[#23232c] overflow-hidden sm:rounded-r-lg`}>
      ...
      <div className='grow flex flex-col  overflow-auto contenedor-chat'>
        {
          ...
          (<div key={id} className={`my-2 p-2 flex ${(userId === host._id ? 'justify-end' : 'justify- ')} `}>
            ...
            <span className={` p-2 px-4 text-sm text-white ${(userId === host._id ? 'rounded-l-xl rounded-tr-xl bg-green-700' : 'rounded-r-xl rounded-tl-xl bg-blue-500')}`}> {body}</span>
